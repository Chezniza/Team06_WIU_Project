...
public class InventoryUI : MonoBehaviour
{
    ...
    // Input actions — no toggle, inventory only opened via shortcut wheel
    ...
    private void Start()
    {
        // Safely get actions — won't crash if action name doesn't exist
        _rotateAction = _playerInput.actions.FindAction("RotateItem");
        _clickAction = _playerInput.actions.FindAction("Click");
    ...
    private void Update()
    {
        ...
        if (_rotateAction != null && _rotateAction.WasPressedThisFrame())
        {
        ...
        if (_clickAction != null && _clickAction.WasPressedThisFrame()) OnLeftClick();
        if (_clickAction != null && _clickAction.WasReleasedThisFrame()) OnRelease();
        if (_rightClickAction != null && _rightClickAction.WasPressedThisFrame()) OnRightClick();
    ...
    // Public — called by ShortcutWheel via UnityEvent
